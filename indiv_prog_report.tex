...
machine shop specs.  After discussion with the team and TA, we moved to a
2-layer stackup for lower fabrication cost; Ingi led the 2-layer revision,
carrying forward my v0 placement and routing decisions.
...
point (SSID: \texttt{BetaSpray}, WPA2), creating a local network that the
host PC connects to directly without an external router.
...
I proposed using the ESP32-S3's internal SPI flash for persistent route
storage early in the design phase.  The \texttt{fatfs.c} module provides a
FatFS filesystem on top of ESP-IDF's wear-levelling layer.  A custom
...
over the DVP parallel interface.  Key configuration decisions:
...
\begin{enumerate}
  \item \textbf{Route CRUD}: routes are stored as binary files on FatFS at
        \texttt{/fatfs/routeN.xy}.  The binary format is a 4-byte hold count
        (little-endian \texttt{uint32}) followed by interleaved
        \texttt{float32} $(x, y)$ pixel coordinates.  This compact format
        (maximum 516 bytes for 64 holds) enables fast read/write without JSON
        parsing overhead on the microcontroller.
